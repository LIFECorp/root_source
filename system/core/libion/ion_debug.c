...
inline
int get_backtrace_show(int *fp, intptr_t* addrs, size_t max_entries)
{
    stack_crawl_state_t state;
    unsigned int bt_num,i; 
    unsigned int **address;
    state.count = max_entries;
    state.addrs = (intptr_t*)addrs;
    _Unwind_Backtrace(trace_function, (void*)&state);
    bt_num = max_entries - state.count;
    address = addrs;
    ALOGE("INIT backtrace totoal %d\n",bt_num);
    for(i=0;i < bt_num;i++)
    {
	ALOGE("INIT [bt %d]0x%x\n",i,address[i]);
    } 
    return max_entries - state.count;
}
...
static map_info_t* parse_maps_line(const char* line)
{
	...
	mi->is_global = false;
        ...
        mi->name[name_len] = '\0';
        //ALOGE("Parsed map: start=0x%08x, end=0x%08x, "
...
void release_my_map_info_list() {
 ...
 //   ALOGE("[release_my_map_info_list] ion_debug_lock 0x%x\n",&ion_debug_lock);
	...
	{
		ALOGE("[ERROR] release g_milist but milistcount is equal or less than zero\n");
	}
	else
	{
		milistcount--;
