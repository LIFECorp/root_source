...
static void lcdc_screen_cap(void)
{
    ...
    if (base) 
    {
        struct fb_overlay_layer_info layer_info;
        
        if(ioctl(fb, MTKFB_GET_OVERLAY_LAYER_INFO, (unsigned long)&layer_info) < 0)
        {
            ALOGI("[DDMSCap]ioctl of MTKFB_GET_OVERLAY_LAYER_INFO fail\n");

            ////mt6577/75 not support MTKFB_GET_OVERLAY_LAYER_INFO, so we set a default orientation value if ioctl failed
            layer_info.layer_rotation = MTK_FB_ORIENTATION_0;
        }

        switch(layer_info.layer_rotation)
        {
            case MTK_FB_ORIENTATION_0:
                write(fd, &w, 4);
                write(fd, &h, 4);
                write(fd, &f, 4);			
//                write(fd, base, size);			
