...
static void fuse_init(struct fuse *fuse, int fd, const char *source_path,
    ...
    if (statfs(fuse->root.name, &stat) < 0) {
		...
		LOG("[fuse_debug]fuse.free_size (init)=%lld \n",fuse->free_size);
    }
#endif
}
        ...
        const void* buffer)
{
	...
	if(!strncmp( fuse->root.name,"/data/media",fuse->root.namelen)){
		...
		if(fuse->free_size <= internal_sdcard_free_size_threshold){
			...
			//handle threshold
			if(fuse->free_size - req->size <= internal_sdcard_free_size_threshold){
				errno = ENOSPC;
...
int main(int argc, char **argv)
{
    ...
    LOG("source_path='%s', dest_path='%s', derive=%d, write_gid=%d \n", source_path, dest_path, derive, write_gid);

#ifdef MTK_SDCARDFS
    { 
        char mountData[255];       
        sprintf(mountData, "derive=%s,write_gid=%d", derive ==  DERIVE_LEGACY ? "legacy" : "unified", write_gid);        
        res = mount(source_path, dest_path, "sdcardfs", 0, mountData);
        if (res) {
            ERROR("mount sdcardfs fail, %s(%d) \n", strerror(errno),errno);
        }
