...
public:
    ...
    ResponseCode readBlob(const char* filename, AES_KEY *aes_key, State state) {
        ALOGW("reading blob %s", filename);
        int in = TEMP_FAILURE_RETRY(open(filename, O_RDONLY));
        if (in < 0) {
            ALOGW("readBlob:open %d:%s", in, strerror(errno));
        ...
        if (close(in) != 0) {
            ALOGW("readBlob:close %s", strerror(errno));
            return SYSTEM_ERROR;
        }

        if (isEncrypted() && (state != STATE_NO_ERROR)) {
            ALOGW("readBlob:state (%d, %d)", state, isEncrypted());
            return LOCKED;
        }

        size_t headerLength = (mBlob.encrypted - (uint8_t*) &mBlob);
        ALOGW("readBlob:length (%x %x)", headerLength, fileLength);
...
public:
    ...
    int32_t get(const String16& name, uint8_t** item, size_t* itemLength) {
        ...
        ResponseCode responseCode = mKeyStore->getKeyForName(&keyBlob, name8, callingUid,
                TYPE_GENERIC);
        if (responseCode != ::NO_ERROR) {
            ALOGW("Could not read %s (%d)", name8.string(), responseCode);
            *item = NULL;
            *itemLength = 0;
            return responseCode;
        }
    ...
    int32_t del(const String16& name, int targetUid) {
        ...
        ResponseCode responseCode = mKeyStore->get(filename.string(), &keyBlob, TYPE_GENERIC,
                callingUid);
        if (responseCode != ::NO_ERROR) {
            ALOGW("Could not del %s (%d)", name8.string(), responseCode);
            //return responseCode;
        }
        ResponseCode rc = (unlink(filename) && errno != ENOENT) ? ::SYSTEM_ERROR : ::NO_ERROR;
        if (rc != ::NO_ERROR) {
            ALOGW("del %s %d(%s)", name8.string(), rc, strerror(errno));
        } else {
            ALOGW("del %s OK", name8.string());
        }

        return (responseCode == ::NO_ERROR) ? (rc) : (responseCode);
    }
    ...
    int32_t exist(const String16& name, int targetUid) {
        ...
        if (access(filename.string(), R_OK) == -1) {
            ALOGW("access error %d(%s)", errno, strerror(errno));
            return (errno != ENOENT) ? ::SYSTEM_ERROR : ::KEY_NOT_FOUND;
        }
        ALOGW("access %s OK", filename.string());
        return ::NO_ERROR;
    }
    ...
    int32_t sign(const String16& name, const uint8_t* data, size_t length, uint8_t** out,
        ...
        ResponseCode responseCode = mKeyStore->getKeyForName(&keyBlob, name8, callingUid,
                ::TYPE_KEY_PAIR);
        if (responseCode != ::NO_ERROR) {
            ALOGW("sign: get %s (%d)", name8.string(), responseCode);
            return responseCode;
        }
    ...
    int32_t verify(const String16& name, const uint8_t* data, size_t dataLength,
        ...
        ResponseCode responseCode = mKeyStore->getKeyForName(&keyBlob, name8, callingUid,
                TYPE_KEY_PAIR);
        if (responseCode != ::NO_ERROR) {
            ALOGW("verify: get %s (%d)", name8.string(), responseCode);
            return responseCode;
        }
    ...
    int32_t get_pubkey(const String16& name, uint8_t** pubkey, size_t* pubkeyLength) {
        ...
        ResponseCode responseCode = mKeyStore->getKeyForName(&keyBlob, name8, callingUid,
                TYPE_KEY_PAIR);
        if (responseCode != ::NO_ERROR) {
            ALOGW("Could not get_pubkey %s (%d)", name8.string(), responseCode);
            return responseCode;
        }
    ...
    int32_t del_key(const String16& name, int targetUid) {
        ...
        ResponseCode responseCode = mKeyStore->get(filename.string(), &keyBlob, ::TYPE_KEY_PAIR,
                callingUid);
        if (responseCode != ::NO_ERROR) {
            ALOGW("Could not del_key %s (%d)", name8.string(), responseCode);
            return responseCode;
        }
    ...
    int32_t duplicate(const String16& srcKey, int32_t srcUid, const String16& destKey,
        ...
        ResponseCode responseCode = mKeyStore->get(sourceFile.string(), &keyBlob, TYPE_ANY,
                callingUid);
        if (responseCode != ::NO_ERROR) {
            ALOGW("Could not dup %s %s (%d)", source8.string(), target8.string(), responseCode);
            return responseCode;
        }
