        ...
        if (is_ipo_reboot) {
                ...
                if (fs_mgr_do_tmpfs_mount(DATA_MNT_POINT)) {
                    ...
                    SLOGD("Successful: mount tmpfs for '%s'\n", DATA_MNT_POINT);
                     if (!access("/protect_s", F_OK) && fs_mgr_do_tmpfs_mount("/protect_s")) {
                        SLOGE("Fail: mount tmpfs for '/protect_s' , errno=%d\n", errno);
                        goto cryptfs_restart_error;
                     }
                     if (!access("/protect_f", F_OK) && fs_mgr_do_tmpfs_mount("/protect_f")) {
               ...
               if (fs_mgr_do_mount(fstab, DATA_MNT_POINT, crypto_blkdev, 0)) {
                    ...
                    SLOGD("Successful: mount crypto_blkdev\n");
                     if (!access("/protect_s", F_OK) && fs_mgr_do_mount(fstab, "/protect_s", "/emmc@protect_s", 0)) {
                         SLOGE("Fail: mount /protect_s, errno=%d\n", errno);
                         goto cryptfs_restart_error;
                     }

                     if (!access("/protect_f", F_OK) && fs_mgr_do_mount(fstab, "/protect_f", "/emmc@protect_f", 0)) {
        ...
        if (is_ipo_reboot) {
          ...
          if (fd >= 0) {
            ...
            if (isBackupBootReason) {
               char buf[8];
               snprintf(buf, sizeof(buf), "%d", BackupBootReason); 
