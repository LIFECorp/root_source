...
DirectVolume::DirectVolume(VolumeManager *vm, const fstab_rec* rec, int flags) :
    ...
    if (strcmp(rec->mount_point, "auto") != 0) {
              ...
              if(!strcmp(rec->label, "icusb1") || !strcmp(rec->label, "icusb2")){                         
			    ALOGE("Vold managed volumes have specified mount point '%s' for MTK_ICUSB_SUPPORT",
			          rec->mount_point);
              }
              else 
        #endif
...
int DirectVolume::handleBlockEvent(NetlinkEvent *evt) {
    ...
    for (it = mPaths->begin(); it != mPaths->end(); ++it) {
        ...
        }else if(!strcmp(getLabel(), "icusb2")){
            if (!strncmp(dp, *it, strlen(*it))){
