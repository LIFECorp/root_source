...
int VolumeManager::listVolumes(SocketClient *cli) {
    ...
    for (i = mVolumes->begin(); i != mVolumes->end(); ++i) {
        ...
        if((*i)->getUuid()){
            char* Uuidvalue;
            Uuidvalue=strdup((*i)->getUuid());
            (*i)->setUuid(Uuidvalue);
			free(Uuidvalue);
        }
        if((*i)->getUserLabel()){
            char* UserLabelvalue;
            UserLabelvalue=strdup((*i)->getUserLabel());
            (*i)->setUserLabel(UserLabelvalue);
			free(UserLabelvalue);
        }
...
#ifdef MTK_ICUSB_SUPPORT
        /*
        if (!strcmp(v->getLabel(), "icusb1")) {// sdb
            SLOGD("Dont share 'icusb1(sdb)', skip this");
            //v->handleVolumeShared();
            return 0;
        }
        else */
        if (!strcmp(v->getLabel(), "icusb2")) {// sda, share to PC 
...
int VolumeManager::unshareVolume(const char *label, const char *method) {
    ...
    else
    {
        ...
        if (!strcmp(v->getLabel(), "icusb2")) {// sda
            if ((fd = open(MASS_STORAGE_ICUSB_FILE_PATH, O_WRONLY)) < 0) {
                SLOGE("Unable to open ums lunfile (%s)", strerror(errno));
                return -1;
            }
