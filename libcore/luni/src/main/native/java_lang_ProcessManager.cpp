...
#if 0 //wschen 2013-11-27 should not call system call which will alloc memory after fork, eg. opendir()
...
#else
static void CloseNonStandardFds(int status_pipe_fd, int SystemPropertiesFd) {
    rlimit rlimit;
    getrlimit(RLIMIT_NOFILE, &rlimit);
    const int max_fd = rlimit.rlim_max;
    for (int fd = 3; fd < max_fd; ++fd) {
        if (fd != status_pipe_fd && fd != SystemPropertiesFd) {
            close(fd);
        }
    }
}
...
#if 1 //wschen 2013-11-27 should not call system call which will alloc memory after fork, eg. opendir()
    // Keep track of the system properties fd so we don't close it.
