...
static inline void swapShorts(jshort* dstShorts, const jshort* srcShorts, size_t count) {
    ...
    if ((reinterpret_cast<uintptr_t>(dst) & SHORT_ALIGNMENT_MASK) == 0 &&
        (reinterpret_cast<uintptr_t>(src) & SHORT_ALIGNMENT_MASK) == 0) {
...
static inline void swapLongs(jlong* dstLongs, const jlong* srcLongs, size_t count) {
    ...
    if ((reinterpret_cast<uintptr_t>(dstLongs) & LONG_ALIGNMENT_MASK) == 0 &&
        (reinterpret_cast<uintptr_t>(srcLongs) & LONG_ALIGNMENT_MASK) == 0) {
        for (size_t i = 0; i < count; ++i) {
          jint v1 = *src++;
          jint v2 = *src++;
          *dst++ = bswap_32(v2);
