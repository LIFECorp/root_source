...
ifeq (true,$(WITH_JAZZ))
# Running Jazz AOT needs to setup host dalvikvm which requires odex file for bootjars
# to be generated
...
jazzaot_boot_jar_module := $(filter $(LOCAL_MODULE),$(JAZZAOT_BOOT_JARS_MODULES))
built_jex :=

ifneq ($(jazzaot_boot_jar_module),)
# boot jar's rules are defined in jazz_aot.mk
jazzaoted_boot_jex := $(JAZZAOT_BOOT_JEX_DIR_FULL_PATH)/$(jazzaot_boot_jar_module).jar.jex

built_jex := $(LOCAL_BUILT_MODULE).jex
$(built_jex) : $(jazzaoted_boot_jex) | $(ACP)
	$(call copy-file-to-target)

$(LOCAL_BUILT_MODULE) : $(built_jex)

# TODO: Consider move the following rules to base_rules.mk
installed_jex := $(LOCAL_INSTALLED_MODULE).jex
$(installed_jex) : $(built_jex) $(LOCAL_BUILT_MODULE) | $(ACP)
	@echo "Install: $@"
	$(copy-file-to-target)

$(LOCAL_INSTALLED_MODULE) : $(installed_jex)

endif # jazzaot_boot_jar_module
